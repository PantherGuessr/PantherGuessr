    ...
    for (const pr of prList.data) {
      if (pr.merged_at) {
        ...
        if (previousTag) {
          ...
          const tagDate = previousTag
            ? (await octokit.rest.git.getRef({ owner: repoOwner, repo: repoName, ref: `tags/${previousTag}` })).data
                .object.tagger?.date
            : null; // Optional chaining for tagger
          const mergedDate = new Date(pr.merged_at);
          const tagDateTime = tagDate ? new Date(tagDate) : new Date(0); // If no tag, consider tag time as epoch
          if (!previousTag || mergedDate > tagDateTime) {
            // Include if no tag or merged after tag
            mergedPRs.push(pr);
          } else if (previousTag && mergedDate <= tagDateTime) {
